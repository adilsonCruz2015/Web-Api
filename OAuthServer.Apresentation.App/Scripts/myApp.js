...
myApp.constant('serviceBasePath', 'http://localhost:25356');
//controllers
...
myApp.factory('dataService', ['$http', 'serviceBasePath', function ($http, serviceBasePath) {
    ...
    fac.GetAnonymousData = function () {
        return $http.get(serviceBasePath + '/api/customers/Entrada').then(function (response) {
            return response.data;
        })
    }

    fac.GetAuthenticateData = function () {
        return $http.get(serviceBasePath + '/api/account').then(function (response) {
            return response.data;
        })
    }

    fac.GetAuthorizeData = function () {
        return $http.get(serviceBasePath + '/api/customers').then(function (response) {
            return response.data;
        })
    }
    return fac;
